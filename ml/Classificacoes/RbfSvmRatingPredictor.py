...
y_pred_treino = modelo.predict(X_treino_scaled)

# ========== FEATURE IMPORTANCE GERAL ==========
# SVM não tem feature importance nativa, usar DecisionTree como aproximação
print("="*80)
print("FEATURE IMPORTANCE GERAL (Aproximação via DecisionTree)")
print("="*80)
dt_approx = DecisionTreeClassifier(max_depth=10, random_state=42, class_weight='balanced')
dt_approx.fit(X_treino, y_treino)

feature_importance = pd.DataFrame({
    'feature': X_treino.columns,
    'importance': dt_approx.feature_importances_
...
print("="*80)
print("FEATURE IMPORTANCE POR CLASSE (Aproximação via DecisionTree)")
print("="*80)
...
plt.title('Matriz de Confusão - TREINO\nRBF SVM', fontsize=14, fontweight='bold')
